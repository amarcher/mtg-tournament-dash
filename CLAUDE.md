...
- **Scheduling has two shapes.** `game_nights` + `night_rsvps` are the standing calendar (a run of dates, each collecting changeable RSVPs plus the plan: set, host, venue); `date_polls` + `poll_options` + `poll_votes` are the Doodle-style "pick one of these candidates" flow. They're complements, not replacements — don't fold one into the other. Both promote into an event through a uniquely-indexed `events.source_night_id` / `source_poll_id`. Date math for recurrences is in `src/lib/recurrence.ts` (pure, wall-time, unit-tested) and pairs with `parseDateTimeLocal` / `toDateTimeLocal` in `src/lib/schedule-types.ts`.
